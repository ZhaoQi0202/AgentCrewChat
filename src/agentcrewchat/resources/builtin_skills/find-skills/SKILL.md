...
- 仅写入 **当前任务根目录** 下：`<任务根>/.agentcrewchat/skills/<skill-id>/`（内含 `SKILL.md`）。
- **不得**把任务级技能写入 `data/skills_install/`；该目录仅用于**应用级**（技能管理或内置默认副本）。

## 实现约定

- 安装副本请调用运行时 API：`agentcrewchat.skills.skill_import.install_skill_roots_to_task_workspace`（由编排层注入 `task_workspace`）。
- 执行任意 Agent 时，可用 `agentcrewchat.skills.registry.merged_skills_for_agents(task_workspace)` 得到 **应用级 + 当前任务级** 全部已启用技能（同名时任务级覆盖应用级）。
