...
from agentcrewchat import paths
from agentcrewchat.bootstrap import ensure_layout
from agentcrewchat.graph.nodes.stubs import (
    architect,
    consultant,
    experts,
    hitl_blueprint,
...
def build_graph(install_root: Path | None = None) -> Any:
    prev = os.environ.get("AGENTCREWCHAT_ROOT")
    if install_root is not None:
        os.environ["AGENTCREWCHAT_ROOT"] = str(install_root.resolve())
    try:
        ensure_layout()
        ...
        if install_root is not None:
            if prev is None:
                ...
                os.environ["AGENTCREWCHAT_ROOT"] = prev

    conn = sqlite3.connect(str(db_path), check_same_thread=False)
    checkpointer = SqliteSaver(conn)

    g = StateGraph(AgentCrewChatState)
